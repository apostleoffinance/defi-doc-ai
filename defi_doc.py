...
# Initialize groq
def initialize_groq(api_key=None):
    if api_key is None:
...
def get_groq_response_with_memory(client, context, question, conversation_history,model_name="llama-3.1-8b-instant"):
    prompt = f"""
        Based on the following context, answer the question as truthfully as possible, and if the answer is not contained within the text below, say "I don't know".
        Context: {context}
        Question: {conversation_history}
        Answer: Provide a clear, accurate answer based only on the information in the context.PermissionError
        """
            ...
            try:
                with st.spinner("🤔 Thinking... (using conversation context + Groq's lightning-fast API)"):
                    ...
                    conversation_history = manage_conversation_context(
                        st.session_state.conversation_history, 
                        max_exchanges=st.session_state.max_history  # <-- Use sidebar value
                    )
...
def main():
    ...
    # Initialize clients
    groq_client = initialize_groq(groq_api_key or None)
